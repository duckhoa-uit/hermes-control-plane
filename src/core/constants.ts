...
export const HEARTBEAT_TIMEOUT_MS = 15 * 60_000; // 15 min — long enough that E2B's onTimeout=15min pause doesn't get mistakenly flagged as a dead runner; short enough that a real runner crash is detected well before the launcher's 24 h hard deadline (src/launcher/server.ts:75). Codex picks 10min; OpenHands picks none (LRU). 15min splits the middle.
