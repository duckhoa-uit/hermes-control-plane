...
//     LAUNCHER_URL is set. This is the long-term mode.
...
async function pollStatus(sessionId: string): Promise<void> {
  ...
  while (Date.now() < deadline) {
    ...
    if (data.session.status === "review_ready" && !prTriggered) {
      ...
      const prResp = await fetch(`${WORKER_URL}/sessions/${sessionId}/create-pr`, {
        method: "POST",
...
async function runSidecarMode(): Promise<void> {
  log(`mode=sidecar url=${LAUNCHER_URL}`);
  const r = await fetch(`${LAUNCHER_URL}/sessions`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
async function runDirectMode(): Promise<void> {
  ...
  const createResp = await fetch(`${WORKER_URL}/sessions`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
