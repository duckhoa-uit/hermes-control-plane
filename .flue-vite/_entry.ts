...
function normalizeBuiltModules(agentModules, workflowModules, channelModules = {}) {
  ...
  for (const [name, mod] of Object.entries(agentModules)) {
    if (
      !mod.default ||
      mod.default.__flueAgentDefinition !== true ||
    ...
    if (previous)
      throw new Error(
        '[flue] Agents "' +
          ...
          '" and "' +
          name +
          '" default-export the same agent definition value. Use distinct defineAgent(...) values for dispatchable agent modules.',
      );
  ...
  for (const [name, mod] of Object.entries(workflowModules)) {
    ...
    if (mod.route !== undefined && typeof mod.route !== "function")
      throw new Error(
        '[flue] Workflow "' + name + '" route export must be a callable Hono middleware value.',
      );
    if (mod.runs !== undefined && typeof mod.runs !== "function")
      throw new Error(
        '[flue] Workflow "' + name + '" runs export must be a callable Hono middleware value.',
      );
    ...
    if (previous)
      throw new Error(
        '[flue] Workflows "' +
          ...
          '" and "' +
          name +
          '" default-export the same workflow definition value. Use distinct defineWorkflow(...) values for workflow modules.',
      );
  ...
  for (const [name, mod] of Object.entries(channelModules)) {
    ...
    if (!channel || typeof channel !== "object" || Array.isArray(channel))
      throw new Error(
        '[flue] Channel "' +
          name +
          '" must export a created channel as the named "channel" binding.',
      );
    ...
    for (const route of channel.routes) {
      ...
      if (typeof route.method !== "string" || !/^[A-Z]+$/.test(route.method))
        throw new Error(
          '[flue] Channel "' + name + '" route method must contain only uppercase ASCII letters.',
        );
      if (
        ...
        throw new Error(
          '[flue] Channel "' +
            name +
            '" route path must be a non-empty absolute suffix without a query or fragment.',
        );
      const segments = route.path.split("/");
      if (segments.some((segment) => segment === "." || segment === ".."))
        throw new Error(
          '[flue] Channel "' + name + '" route path must remain beneath its channel namespace.',
        );
...
if (
  typeof cloudflareHandlers !== "object" ||
  cloudflareHandlers === null ||
  Array.isArray(cloudflareHandlers)
) {
  throw new Error(
    "[flue] cloudflare.ts default export must be an object containing non-HTTP Worker handlers.",
  );
}
if ("fetch" in cloudflareHandlers) {
  throw new Error(
    "[flue] cloudflare.ts default export must not define fetch. Use app.ts for custom HTTP handling.",
  );
}
...
const dispatchQueue = {
  async enqueue(input) {
    ...
    if (!binding)
      throw new Error(
        '[flue] dispatch() target agent "' +
          input.agent +
          '" Durable Object binding is unavailable.',
      );
    const response = await fetchAgent(
      binding,
      input.id,
      new Request("https://flue.invalid" + INTERNAL_DISPATCH_PATH, {
        method: "POST",
    ...
    if (!response.ok)
      throw new Error(
        '[flue] dispatch() target agent "' +
          input.agent +
          '" rejected durable admission with status ' +
          response.status +
          ".",
      );
...
function createRunStoreForRequest(doInstance) {
  ...
  if (!sql) {
    throw new Error(
      "[flue] Durable Object SQLite storage is unavailable — cannot create the run store. Flue Durable Object classes require SQLite-backed storage.",
    );
  }
...
function createEventStreamStoreForInstance(doInstance) {
  ...
  if (!sql) {
    throw new Error(
      "[flue] Durable Object SQLite storage is unavailable — cannot create the event stream store. Flue Durable Object classes require SQLite-backed storage.",
    );
  }
...
async function handleFlueWorkflowFiberRecovered(ctx, doInstance, workflowName) {
  ...
  await failRecoveredRun({
    ...
    request: new Request("https://flue.invalid/workflows/" + encodeURIComponent(workflowName), {
      method: "POST",
    }),
    error: new Error(
...
async function dispatchWorkflow(request, doInstance, workflowName) {
  ...
  if (
    request.method === "POST" &&
    new URL(request.url).pathname === INTERNAL_WORKFLOW_INVOKE_PATH
  ) {
  ...
  return runWithInstanceContext(doInstance, identity, () =>
    handleWorkflowRequest({
      request,
      workflowName,
      runId: instanceId,
...
const FlueControlPlanAgent = class FlueControlPlanAgent extends agentExtension0.base(Agent) {
  ...
  onStart(props) {
    return cloudflareAgents.onStart(this, () =>
  ...
  onFiberRecovered(ctx) {
    return cloudflareAgents.onFiberRecovered(this, ctx, () =>
...
configureFlueRuntime({
  ...
  admitWorkflow: async ({ workflowName, input }) => {
    ...
    const binding = env?.[identity?.bindingName];
    if (!binding)
      throw new Error("[flue] invoke() target workflow Durable Object binding is unavailable.");
    const runId = generateWorkflowRunId();
    const response = await fetchAgent(
      binding,
      runId,
      new Request("https://flue.invalid" + INTERNAL_WORKFLOW_INVOKE_PATH, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ input }),
      }),
    ...
    if (!response.ok)
      throw new Error(
        "[flue] invoke() target workflow rejected durable admission with status " +
          response.status +
          ".",
      );
    return response.json();
